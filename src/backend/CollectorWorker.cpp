...
void CollectorWorker::start() {
	...
	m_timer.setInterval(TopConfig::instance()->pollIntervalMs());
	m_timer.start();
	...
	//? Queued to the TopConfig object (GUI thread) to keep the notifier thread-safe.
	auto* settings = TopConfig::instance();
	if (Cpu::got_sensors)
...
void CollectorWorker::tick() { //? tick counter for slow cadences (fdinfo walk every 2nd tick)
	...
	try {
		...
		const bool ioReads = TopConfig::getB("proc_io_reads");
	...
	try {
		if (TopConfig::getB("gpu_fdinfo_walk"))
	...
	ss.batteryAvailable = Cpu::has_battery and TopConfig::getB("show_battery");
	if (Cpu::has_battery and TopConfig::getB("show_battery")) {
	...
	ss.cpuWattsAvailable = Cpu::supports_watts and TopConfig::getB("show_cpu_watts");
