...
namespace Disk {
	...
	void collect(uint64_t swap_total, uint64_t swap_used, uint64_t swap_free,
		...
		auto show_disks = TopConfig::getB("show_disks");
		if (not show_disks) return; //? stolen gate (btop 2425)

		try {
			const auto& disks_filter = TopConfig::getS("disks_filter");
			bool filter_exclude = false;
			auto use_fstab = TopConfig::getB("use_fstab");
			auto only_physical = TopConfig::getB("only_physical");
			auto zfs_hide_datasets = TopConfig::getB("zfs_hide_datasets");
			ifstream diskread;
			...
			if (diskread.good()) {
				...
				if (TopConfig::getB("swap_disk") and has_swap) found.push_back("swap");
			...
			for (auto it = disks.begin(); it != disks.end(); ) {
				...
				const auto free_priv = TopConfig::getB("disk_free_priv");
			...
			if (TopConfig::getB("swap_disk") and has_swap) {
...
namespace Net {
	...
	auto collect() -> net_info& { //? btop 2891–3082; Runner::stopping removed
		...
		const auto& config_iface = TopConfig::getS("net_iface");
		auto net_sync = TopConfig::getB("net_sync");
		auto net_auto = TopConfig::getB("net_auto");
