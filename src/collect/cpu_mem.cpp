 ...
 *
 * Naturalization (phase-2 checklist): Config::getS/getB → TopConfig::getS/getB;
...
namespace Shared {
	...
	void init() {
		...
		const auto shown_gpus = TopConfig::getS("shown_gpus");
...
namespace Cpu {
	...
	auto get_core_mapping() -> std::unordered_map<int, int> { //? btop 760–826
		...
		const auto& custom_map = TopConfig::getS("cpu_core_map");
	...
	string get_cpuHz() { //? btop 669–758 (freq_mode key at 677)
		...
		const auto& freq_mode = TopConfig::getS("freq_mode");
	...
	auto collect() -> cpu_info& { //? btop 1104–1247; no_update/Runner::stopping removed
		...
		if (TopConfig::getB("show_cpu_freq"))
			cpuHz = get_cpuHz();
		...
		if (TopConfig::getB("check_temp") and got_sensors)
			update_sensors();

		if (TopConfig::getB("show_battery") and has_battery)
			current_bat = get_battery();

		if (TopConfig::getB("show_cpu_watts") and supports_watts)
...
namespace Mem {
	...
	auto collect() -> mem_info& { //? btop 2333–2423 (pure-memory section; show_swap/swap_disk/zfs_arc_cached → TopConfig)
		auto show_swap = TopConfig::getB("show_swap");
		auto swap_disk = TopConfig::getB("swap_disk");
		auto zfs_arc_cached = TopConfig::getB("zfs_arc_cached");
