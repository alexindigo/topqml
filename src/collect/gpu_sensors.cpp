	...
	namespace Nvml {
		...
		bool collect(gpu_info* gpus_slice) {
			...
			for (unsigned int i = 0; i < device_count; ++i) {
				...
				if (gpus_slice[i].supported_functions.pcie_txrx and (TopConfig::getB("nvml_measure_pcie_speeds") or is_init)) {
					unsigned int tx;
	...
	namespace Rsmi {
		...
		bool collect(gpu_info* gpus_slice) {
			...
			for (uint32_t i = 0; i < device_count; ++i) {
				...
				//? GPU temperature
				if (gpus_slice[i].supported_functions.temp_info) {
					if (TopConfig::getB("check_temp") or is_init) {
						int64_t temp;
				...
				if ((gpus_slice[i].supported_functions.pcie_txrx and TopConfig::getB("rsmi_measure_pcie_speeds")) or is_init) {
					uint64_t tx, rx;
...
namespace Cpu {
	...
	void update_sensors() { //? btop 624–647
		...
		const auto cfg_sensor = sstr(TopConfig::getS("cpu_sensor"));
		...
		if (TopConfig::getB("show_coretemp") and not cpu_temp_only) {
	...
	auto get_battery() -> tuple<int, float, long, string> {
		...
		const auto battery_sel = sstr(TopConfig::getS("selected_battery"));
