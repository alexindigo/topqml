...
namespace Proc {
	...
	static void _collect_details(const size_t pid, const uint64_t uptime, vector<proc_info>& procs) {
		...
		if (pid != detailed.last_pid) {
			...
			detailed.skip_smaps = not TopConfig::getB("proc_info_smaps");
		}
		...
		if (not TopConfig::getB("proc_per_core")) detailed.entry.cpu_p *= Shared::coreCount;
	...
	//* 3537–3620 NOT stolen; Runner::stopping removed; Config → TopConfig)
	auto collect(bool no_update) -> vector<proc_info>& {
		const auto sorting = sstr(TopConfig::getS("proc_sorting"));
		auto reverse = TopConfig::getB("proc_reversed");
		const auto& filter = sstr(TopConfig::getS("proc_filter"));
		auto per_core = TopConfig::getB("proc_per_core");
		auto should_filter_kernel = TopConfig::getB("proc_filter_kernel");
		auto show_detailed = detailed_pid.load() != 0;
		const size_t v_detailed_pid = detailed_pid.load();
		const auto pause_proc_list = TopConfig::getB("pause_proc_list");
		...
		//* ---------------------------------------------Collection start----------------------------------------------
			...
			//? Set correct state of dead processes if paused
				...
				const bool keep_dead_proc_usage = TopConfig::getB("keep_dead_proc_usage");
